  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.85)' }}>
      <div className="relative w-full max-w-2xl mx-4">
        ...
        <div className="relative bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 border-2 border-amber-500/50 rounded-2xl p-6 shadow-2xl">
          <div className="text-center mb-4">
            <h2 className="text-3xl font-black text-amber-400 tracking-wider" style={{ textShadow: '0 0 20px rgba(245,158,11,0.5)' }}>
              ASTA
            </h2>
            <p className="text-amber-200/70 text-sm mt-1">Giocata da {auctionData.initiator}</p>
          </div>
            ...
            <div className="flex-shrink-0 text-center">
              <div className="relative">
                ...
                <img
                  ...
                  className="relative w-40 h-56 object-cover rounded-lg border-2 border-amber-400/50 shadow-lg"
                />
              </div>
              <h3 className="text-white font-bold text-lg mt-3">{auctionData.cardName}</h3>
              <div className="flex gap-3 justify-center mt-1">
                <span className="text-cyan-400 text-sm font-bold">PTI: {auctionData.cardPti}</span>
                <span className="text-yellow-400 text-sm font-bold">Stelle: {auctionData.cardStars}</span>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-3">
              <div className="bg-black/40 rounded-xl p-4 border border-amber-500/30">
                <div className="text-center">
                  <p className="text-amber-200/60 text-xs uppercase tracking-wider">Offerta attuale</p>
                  <p className="text-4xl font-black text-amber-400 mt-1" style={{ textShadow: '0 0 15px rgba(245,158,11,0.4)' }}>
                ...
                {currentBid > 0 && !ended && (
                  <div className="mt-3 text-center">
                    <p className="text-xs text-white/50 uppercase tracking-wider">Aggiudicazione tra</p>
                    <p className={`text-5xl font-black ${countdownColor} mt-1 transition-all`} style={{ textShadow: '0 0 20px currentColor' }}>
                      {countdown}
                    </p>
                  </div>
                )}
                ...
                {currentBid === 0 && !ended && (
                  <p className="text-amber-200/50 text-center text-sm mt-2 animate-pulse">In attesa di offerte...</p>
                )}
              </div>

              <div ref={historyRef} className="bg-black/30 rounded-lg p-2 max-h-24 overflow-y-auto border border-white/10">
                {bidHistory.length === 0 ? (
                  <p className="text-white/30 text-xs text-center py-2">Nessuna offerta ancora</p>
                ) : (
                  bidHistory.map((bid, i) => (
                    <div key={i} className="flex justify-between items-center py-1 px-2 text-xs border-b border-white/5 last:border-0">
                ...
                <div className="bg-black/30 rounded-xl p-3 border border-white/10">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-black/50 border border-amber-500/30 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-400"
                      min={currentBid + 1}
                      max={myRankiard}
                    />
                    <Button
                      ...
                      className="bg-amber-600 hover:bg-amber-500 text-white font-bold px-4 py-2 disabled:opacity-40"
                    >
                      OFFRI
                    </Button>
                  </div>
              ...
              {ended && auctionResult && (
                <div className="bg-black/50 rounded-xl p-4 border border-amber-500/40 text-center">
                  {auctionResult.winner ? (
                    <>
                      ...
                      <p className="text-2xl font-black text-amber-400 mt-2">AGGIUDICATO!</p>
                    ...
                    <>
                      <p className="text-2xl text-white/60">Asta deserta</p>
                      <p className="text-white/40 text-sm mt-1">{auctionResult.message}</p>
                    </>
                  )}
                </div>
              )}
