...
export function GymMode({ playerName, userId, avatarId, onBack, pendingGymGame, onResumeGymGame }: GymModeProps) {
  ...
  const handleInternalResume = useCallback((leader: GymLeader, gameIdToResume: string) => {
    ...
    // CRITICAL: Do NOT emit rejoin-game here! The server will auto-rejoin us when
    // it receives set-user-data (which includes authToken). If we emit rejoin-game
    // before socket is authenticated, it gets rejected by the server for security.
    // Just listen for game-state-update from the auto-rejoin.
    ...
    const handleStateUpdate = () => {
      if (!stateUpdateHandled) {
        ...
        console.log('[GymMode] Game-state-update received from server auto-rejoin, switching to battle');
        setPhase('battle');
    ...
    fallbackTimer = setTimeout(() => {
      if (!stateUpdateHandled) {
        stateUpdateHandled = true;
        console.warn('[GymMode] game-state-update not received in time (server auto-rejoin may have completed), showing battle anyway');
        setPhase('battle');
        socket.off('game-state-update', handleStateUpdate);
      }
    }, 4000);
