...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  io.on('connection', (socket) => {
      ...
      try {
        ...
        if (alreadyJoinedGameId) {
          ...
          if (alreadyJoinedGame && !alreadyJoinedGame.gameEnded && alreadyJoinedGame.players[playerName]) {
            ...
            const gymLeaderId = alreadyJoinedGame.gymLeaderId;
            const fantaTournamentId = alreadyJoinedGame.fantaTournamentId;
            console.log(`🔒 Socket already in game ${alreadyJoinedGameId} (via set-user-data), returning it instead of hint ${lastGameId}`);
            socket.emit('active-game-found', { gameId: alreadyJoinedGameId, handCount, playerName, gameMode, gymLeaderCpuName, gymLeaderId, fantaTournamentId });
            return;
          }
        }
                            ...
                            if (attackResult.result?.barrieraAbsorbed) {
                              ...
                              setTimeout(() => {
                                ...
                                if (nextAfterCPU) {
                                  ...
                                  } else if (freshGameAfterBarriera?.helpEnabled) {
                                    const helpCtx = buildHelpContext(freshGameAfterBarriera, nextAfterCPU);
    ...
    socket.on('choice-panel-response', async ({ choiceId, value, gameId: gId }: { choiceId: string; value: string; gameId: string }) => {
      ...
      if (pendingFotografo && pendingFotografo.choiceId === choiceId && pendingFotografo.playerName === socketPlayerName) {
        ...
        const chosenType = value; // 'bonus' | 'mosse' | 'personaggi' | 'personaggi_speciali'
        const opponents = game.turnOrder.filter((p: string) => p !== socketPlayerName && !game.eliminatedPlayers?.has(p));
        const revealedCards: Array<{ owner: string; card: any }> = [];
        for (const opp of opponents) {
          const oppHand = game.players[opp]?.hand || [];
          const matching = oppHand.filter((c: any) => c.type === chosenType || (chosenType === 'personaggi' && c.type === 'personaggi_speciali'));
          if (matching.length > 0) {
            const revealed = matching[Math.floor(Math.random() * matching.length)];
            revealedCards.push({ owner: opp, card: revealed });
          }
        }
        const playerSocketId = (game.players[socketPlayerName] as any)?.socketId;
        if (playerSocketId && revealedCards.length > 0) {
          io.to(playerSocketId).emit('fotografo-reveal-hand', {
            playerName: 'multiple',
            hand: revealedCards.map(rc => ({ id: rc.card.id, name: rc.card.name, frontImage: rc.card.frontImage, type: rc.card.type, effect: rc.card.effect, owner: rc.owner }))
          });
        }
        const revealSummary = revealedCards.length > 0
          ? revealedCards.map(rc => `${rc.owner}: ${rc.card.name || rc.card.type || 'carta'}`).join(', ')
          : 'nessuna carta trovata';
        io.to(gameId).emit('chat-message', {
          id: `${Date.now()}-fotografo-reveal`,
          playerName: 'Sistema',
          message: `📸 OCCHIO DEL FOTOGRAFO! ${socketPlayerName} rivela carte "${chosenType}": ${revealSummary}`,
          timestamp: Date.now()
        });
        io.to(gameId).emit('game-state-update', gameManager.getSanitizedGameState(gameId));
        return;
      ...
      // ── PLAYBACK ─────────────────────────────────────────────────────────────
      // Player chose opponent character to sample stars from; sets game.playbackStars[playerName].
      ...
      if (pendingPB && pendingPB.choiceId === choiceId && pendingPB.playerName === socketPlayerName) {
        ...
        const sampledChar = game.field?.find((c: any) => c.id === value);
        if (sampledChar) {
          let pbSampledStars = sampledChar.stars || (gameManager as any).extractStarsFromNote(sampledChar.text || '') || 1;
          if (pendingPB.hasGigione) pbSampledStars = pbSampledStars * 2;
          if (!(game as any).playbackStars) (game as any).playbackStars = {};
          (game as any).playbackStars[socketPlayerName] = pbSampledStars;
          io.to(gameId).emit('chat-message', {
            id: `${Date.now()}-playback-sampled`,
            playerName: 'Sistema',
            message: `🎬 PLAYBACK! ${socketPlayerName} campiona ${sampledChar.name || sampledChar.owner} (${pbSampledStars}★${pendingPB.hasGigione ? ' ×2 GIGIONE' : ''}). Il prossimo attacco varrà ${pbSampledStars * 50} PTI!`,
            timestamp: Date.now()
          });
        }
        io.to(gameId).emit('game-state-update', gameManager.getSanitizedGameState(gameId));
                ...
                if (cpuAction) {
                  ...
                  setTimeout(() => {
                    ...
                    if (nextAfterCPU) {
                      ...
                      } else if (freshGameState?.helpEnabled) {
                        const helpCtx = buildHelpContext(freshGameState, nextAfterCPU);
                        generateHelpMessage(gameId, 'human_turn_start', helpCtx).then(helpMsg => {
                          emitHelpMessage(io, gameId, helpMsg);
                        });
                      }
  ...
  app.post('/api/admin/sync-databases', authMiddleware, async (req, res) => {
    try {
      ...
      // getPrimaryDb() always holds the original primary (EXTERNAL_DATABASE_URL),
      // even when we've auto-switched to fallback mode.
      // getFallbackDb() holds the secondary (DATABASE_URL / Replit).
      ...
      if (primaryDb === fallbackDb) {
        return res.status(400).json({
          success: false,
          error: 'Primary e fallback coincidono — nessuna sincronizzazione necessaria',
        });
      }
      ...
      await syncTable('users', users, r => r['id'] as number, true);
      await syncTable('tournaments', tournaments, r => r['id'] as number, true);
      await syncTable('tournament_participants', tournamentParticipants, r => r['id'] as number, true);
      await syncTable('tournament_matches', tournamentMatches, r => r['id'] as number, true);
      await syncTable('custom_cards', customCards, r => r['id'] as number, true);
      await syncTable('clans', clans, r => r['id'] as number, true);
      await syncTable('clan_members', clanMembers, r => r['id'] as number, true);
      await syncTable('draft_decks', draftDecks, r => r['id'] as number, true);
      await syncTable('user_draft_credits', userDraftCredits, r => r['id'] as number, true);
      await syncTable('user_card_collection', userCardCollection, r => r['id'] as number, true);
...
Rispondi SOLO con JSON, nessun testo fuori dal JSON:
{
  ...
  // Helper: generate round-robin schedule (all pairs, giornate format)
  function buildRoundRobinSchedule(
    ...
    // Ensure even number for polygon method
    const padded = participantIds.length % 2 === 0
      ? [...participantIds]
      : [...participantIds, null]; // null = bye
    const n = padded.length;
    const numRounds = n - 1;
    const fixed = padded[0];
    const rotating = padded.slice(1);
    const matches: { playerIds: (number | null)[]; round: number; matchNumber: number; status: string; winnerId: number | null }[] = [];
    for (let r = 0; r < numRounds; r++) {
      ...
      for (let i = 0; i < n / 2; i++) {
        ...
        const players: (number | null)[] = [p1, p2];
        const realPlayers = players.filter((p): p is number => p !== null);
        const isBye = realPlayers.length < 2;
        matches.push({
          playerIds: players,
          round: r + 1,
          matchNumber: matchNum++,
          status: isBye ? 'completed' : 'pending',
          winnerId: isBye ? realPlayers[0] ?? null : null,
        });
      }
      // Rotate: last element goes to position 1 (after fixed)
  ...
  // GET /api/tournaments/:id/standings — standings for round_robin campionati
  app.get('/api/tournaments/:id/standings', async (req, res) => {
    try {
      ...
      if (humanIds.length) {
        const urows = await db.select({ id: users.id, username: users.username }).from(users).where(inArray(users.id, humanIds));
        urows.forEach(u => { userMap[u.id] = u.username; });
      }
      ...
      const standings = participants
        .map(p => ({
          participantId: p.id,
          userId: p.userId,
          displayName: p.isCpu ? (p.displayName || 'CPU') : (p.userId ? userMap[p.userId] : p.displayName) || 'Giocatore',
          isCpu: p.isCpu,
          wins: p.wins,
          losses: p.losses,
  ...
  app.post('/api/tournaments/:id/start', authMiddleware, async (req, res) => {
    try {
      ...
      const tournamentType = tournament[0].type || 'elimination';

      // Update tournament status
      await db.update(tournaments).set({ status: 'in_progress', startDate: new Date() }).where(eq(tournaments.id, tournamentId));

      let allMatchesToInsert: { playerIds: (number | null)[]; round: number; matchNumber: number; status: string; winnerId: number | null }[] = [];

      if (tournamentType === 'round_robin') {
        // Generate full round-robin calendar (all pairs across giornate)
        allMatchesToInsert = buildRoundRobinSchedule(shuffled);
      } else {
        // Generate round 1 bracket (elimination)
        allMatchesToInsert = buildBracketRound(shuffled, ppm, 1);
      }

      // Insert all matches
      const insertedMatches = [];
      for (const m of allMatchesToInsert) {
      ...
      const successMsg = tournamentType === 'round_robin'
        ? `🏆 Il campionato "${tournament[0].name}" è iniziato! Calendario generato.`
        : `🏆 Il torneo "${tournament[0].name}" è iniziato! Sorteggio completato.`;
      ...
      const resultMsg = tournamentType === 'round_robin' ? 'Campionato avviato! Calendario generato.' : 'Torneo avviato! Tabellone generato.';
      res.json({ success: true, message: resultMsg, matches: updatedMatches });
    } catch (error) {
    ...
    try {
      ...
      for (const loserId of losers) {
        if (loserId > 0) {
          ...
          if (loserPart.length) {
            await db.update(tournamentParticipants)
              .set({
                // In round_robin, losers are NOT eliminated — they keep playing
      ...
      if (tournamentType === 'round_robin') {
        ...
        const allMatches = await db.select().from(tournamentMatches)
          .where(eq(tournamentMatches.tournamentId, matchData.tournamentId));
        const effectiveAll = allMatches.map(m => m.id === matchId ? { ...m, status: 'completed', winnerId } : m);
        const allDone = effectiveAll.every(m => m.status === 'completed');

        if (allDone) {
          // Find winner by points (wins × 2), tiebreaker: wins count
          const participants = await db.select().from(tournamentParticipants)
            .where(eq(tournamentParticipants.tournamentId, matchData.tournamentId));
          // Re-fetch to get updated wins
          const updatedWinner = await db.select().from(tournamentParticipants)
            .where(eq(tournamentParticipants.tournamentId, matchData.tournamentId));
          const sorted = [...updatedWinner]
            .map(p => ({ ...p, points: (p.wins + (p.userId === winnerId ? 1 : 0)) * 2 }))
            .sort((a, b) => b.points - a.points || b.wins - a.wins);
          // Apply winner win update to the in-memory sort
          const sortedWithFinalWin = updatedWinner
            .map(p => ({ ...p, wins: p.wins + (p.userId === winnerId ? 1 : 0) }))
            .map(p => ({ ...p, points: p.wins * 2 }))
            .sort((a, b) => b.points - a.points || b.wins - a.wins);

          const champWinner = sortedWithFinalWin[0];
          const champRunnerUp = sortedWithFinalWin[1];
          const finalWinnerId = champWinner?.userId ?? null;
          const runnerUpId = champRunnerUp?.userId ?? null;
        ...
        // Elimination: check if all matches in current round are completed
  ...
  // GET /api/draft/growth - get user's draft character growth data
  app.get('/api/draft/growth', authMiddleware, async (req, res) => {
    try {
      if (!isDatabaseAvailable()) return res.status(503).json({ error: 'DB non disponibile' });
      const user = (req as any).user;
      const [currentUser] = await db.select().from(users).where(eq(users.email, user.email)).limit(1);
      if (!currentUser) return res.status(404).json({ error: 'User not found' });
      const growthRows = await db.select().from(draftCharacterGrowth).where(eq(draftCharacterGrowth.userId, currentUser.id));
      res.json(growthRows);
    } catch (error) {
      res.status(500).json({ error: 'Errore nel recupero dati crescita' });
    }
  });
