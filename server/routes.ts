...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('choice-panel-response', async ({ choiceId, value, gameId: gId }: { choiceId: string; value: string; gameId: string }) => {
      ...
      // ── PLAYBACK ─────────────────────────────────────────────────────────────
      // Phase 1: choose the opponent's character that will execute the attack.
      // Phase 2: choose the target. Attack fires using the forced attacker's stars × 50 (×2 if GIGIONE).
      ...
      if (pendingPB && pendingPB.choiceId === choiceId && pendingPB.playerName === socketPlayerName) {
        if (pendingPB.phase === 'choose_target') {
          // Phase 2: execute the forced attack
          ...
          if (forcedAttackerChar && targetChar) {
            const attackerStars = forcedAttackerChar.stars || (gameManager as any).extractStarsFromNote(forcedAttackerChar.text || '') || 1;
        ...
        // Phase 1: player chose forced attacker (enemy char), now show target choice
...
Rispondi SOLO con JSON, nessun testo fuori dal JSON:
{
  ...
  app.post('/api/story-mode/reset', authMiddleware, async (req, res) => {
    try {
      ...
      console.error('Error resetting story mode progress:', e);
      res.status(500).json({ success: false, error: 'Errore server' });
    }
  });
