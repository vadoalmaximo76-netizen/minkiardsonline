...
- **Draft Tournament System**: Full "Torneo Draft" mode in DraftSection (tab "Torneo" + tab "Classifica"). Entry cost 100 credits, max 7 wins / 3 losses. Win-scaled rewards (75→750 cr). After each win: `DraftWinReward.tsx` celebration overlay (confetti+trophy), then `DraftCardPickerModal.tsx` animated 5-card flip-reveal picker. Draft Rating (+30 win, -20 loss, floor 0) + public leaderboard. DB: `draftTournaments` table + `draftRating`/`draftBestRun` on users.
