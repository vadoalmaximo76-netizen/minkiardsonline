...
- **Cinematic Attack Overlay** (`CinematicOverlay.tsx`): Full-screen letterbox animation triggered by `cinematic-event` socket for high-damage attacks (≥150 PTI = `big_attack`, ≥300 PTI = `mega_attack`) and special bonus cards. Centralized emit inside `processMosseDamage` in `gameManager.ts` (uses actual resolved damage). `animationType` drives visual theming via `resolveTheme()` — each `mosseDamageEffect` gets distinct colors/emoji/label (death=skull/crimson, halve_pti=poison green, zero_stars=blue, gamble_death=gold dice, etc.). `cinematicFlash` in `GameBoard.tsx` is gated to `dmg < 150` to avoid overlap with overlay. Special bonus emits: kainoken (`gameManager.ts`), SOROS activation (2 paths), Roulette Russa lethal (CPU in `gameManager.ts` + human in `routes.ts`), Corruzione Massiva (CPU path in `gameManager.ts` + human choice resolution in `routes.ts`). Visible to all players and spectators via broadcast socket event.
