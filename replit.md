...
- **Cinematic Attack Overlay** (`CinematicOverlay.tsx`): Full-screen letterbox animation triggered by `cinematic-event` socket for high-damage attacks (≥150 PTI = `big_attack`, ≥300 PTI = `mega_attack`) and special bonus cards (kainoken = `special_bonus`). Shows letterbox bars, attacker character name, animated damage counter, and color-coded theme (orange/red/purple/black). Visible to all players and spectators via broadcast socket event. Server emits from the 4 main attack paths in `routes.ts` and kainoken case in `gameManager.ts`.
